...
e (c) otimização das ferramentas de IA (latência percebida e qualidade). **Entrega
final = versão de TESTE pronta para o usuário final** (ZIP portátil validado + roteiro
de teste atualizado), **sem nenhum ponto em aberto**: toda pendência conhecida é
resolvida internamente pelos agentes nesta rodada. Modificações e ajustes posteriores
ficam para a PRÓXIMA versão, a partir do feedback do usuário.
...
- [ ] **P.4** O que mais a Onda R classificar como perf de leitura.
...
- [ ] **Q.4** O que mais a Onda R classificar como IA (ex.: limiares do feedback 👎
...
- [ ] **S.5** O que mais a Onda R classificar como UX.
...
- Iniciativa mobile/API M0 (`feat/api-m0`): corre em ciclo próprio; esta rodada não
  a toca (worktree garante).
...
> crie o worktree dedicado, rode as ondas R→P→Q→S→T com merge automático autorizado,
> resolva TODAS as pendências internamente (gate: parar só em falha dupla) e entregue
> ao final o ZIP de teste + roteiro do usuário. Sem perguntas intermediárias; testes
> reais ficam com o usuário após a entrega.
