...
class RAGEngine:
    ...
    def query_rag(
        ...
        """Pipeline RAG com loop iterativo de Function Calling (Agentic RAG).

        Retorna um gerador que emite tokens à medida que o LLM os gera
        (streaming). Ideal para uso com QThread + pyqtSignal.

        Args:
            question: Pergunta do usuário.
            n_context: Override do número de chunks de contexto.
            book_id: Filtro de ID do livro para escopo.
            ui_mutation_callback: Callback(action_type: str, data: dict) para
                                  mutações visuais da interface (highlight, bookmark).
                                  Chamado na thread do worker — o receptor deve
                                  garantir thread-safety via sinais Qt.

        Yields:
            Tokens de texto gerados pelo LLM.

        Raises:
            RuntimeError: Se Ollama não disponível ou sem documentos indexados.
        """
        if not self.is_ollama_available():
            raise RuntimeError(
        ...
        try:
            similar = self.search_similar(question, n_results=n, book_id=book_id)
        except Exception as e:
            yield f"\n⚠️ Falha ao buscar contexto vetorial (Embeddings falharam): {e}"
            return
        ...
        # Monta contexto e registra as fontes
        ...
        # Usar API de Chat para suportar Function Calling
        import json
        import urllib.request

        # Garantir ordem imutável no topo para tirar proveito máximo de Prefix Caching
        messages = [
        ...
        def call_chat_api(msgs: list, stream: bool = False, temperature: float | None = None):
            payload_dict = {
                "model": self._llm_model.strip(),
                "messages": msgs,
                "stream": stream,
                "tools": _TOOLS_DEF,
            }
            ...
            endpoint = f"{self._ollama_url.rstrip('/')}/api/chat"
            req = urllib.request.Request(
                endpoint,
                data=payload,
                headers={"Content-Type": "application/json"},
        ...
        def _execute_tool(fn_name: str, args: dict) -> str:
            """Executa a ferramenta chamada pelo LLM e retorna resultado como JSON string."""
            try:
                if fn_name == "search_web":
                    from duckduckgo_search import DDGS
                    query_term = args.get("query", "")
                    results = DDGS().text(query_term, max_results=3)
                    ...
                    hits = self.search_similar(args["query"], n_results=5, book_id=bid)
                    return json.dumps([
                        {
                    ...
                    if isinstance(bid, str):
                        bid = int(bid) if bid.isdigit() else None
                    hits = self.keyword_search_db(args["term"], book_id=bid)
                    ...
                    topic = args["topic"]
                    if cbid and self._collection is not None:
                        try:
                            emb = self._get_embedding(topic)
                            res = self._collection.query(
                                query_embeddings=[emb],
                                n_results=5,
                                where={"book_id": {"$ne": cbid}},
                                include=["documents", "metadatas", "distances"],
                            )
                            formatted = []
                            ...
                            logger.warning("cross_reference ChromaDB failed: %s", exc)
                            # Fallback: busca global filtrando manualmente
                            hits = self.search_similar(topic, n_results=8, book_id=None)
                            return json.dumps([
                                {
                                    "title": h["metadata"].get("title", ""),
                                    "page": (h["metadata"].get("page_number") or 0) + 1,
                                ...
                                for h in hits
                                if h["metadata"].get("book_id") != cbid
                            ], ensure_ascii=False)
                    else:
                        hits = self.search_similar(topic, n_results=5)
                        return json.dumps([
                            {
                                "title": h["metadata"].get("title", ""),
                                "page": (h["metadata"].get("page_number") or 0) + 1,
                ...
                logger.error("Erro na ferramenta '%s': %s", fn_name, exc)
                return json.dumps({"error": f"A ferramenta falhou: {str(exc)}"})

        # ── Loop Iterativo de Tool Calls (Agentic RAG) ──
        MAX_TOOL_ROUNDS = 5
        self._cancelled = False
        ...
        for _round in range(MAX_TOOL_ROUNDS):
            ...
            try:
                # Amostragem com temperature 0.1 para forçar saídas JSON estruturadas determinísticas e rápidas
                with call_chat_api(messages, stream=False, temperature=0.1) as resp:
                    data = json.loads(resp.read())
            except urllib.error.HTTPError as exc:
                body = exc.read().decode("utf-8", errors="replace")
                logger.error("HTTPError %s no Ollama: %s", exc.code, body)
                yield f"\n⚠️ Erro {exc.code} na API do Ollama: {body}"
                return
            except Exception as exc:
                yield f"\n⚠️ Falha ao contactar assistente: {exc}"
                return

            message = data.get("message", {})

            if not message.get("tool_calls"):
                # LLM não quer mais ferramentas → responde (simula streaming)
            ...
            # Há tool_calls → executa cada ferramenta e acumula resultado
        ...
        # Esgotou MAX_TOOL_ROUNDS → força resposta final via stream
        yield "\n[⚠️ Limite de rodadas de ferramentas atingido — gerando resposta final...]\n"
        try:
            with call_chat_api(messages, stream=True) as resp:
                for line in resp:
                    if self._cancelled:
                        return
                    if not line:
