...
Cobre os limiares (≥4 negativos, categoria ≥3, teto de 2), o desempate
canônico, texto livre que não vira categoria, robustez a linhas malformadas
(ADR-005) e a garantia de núcleo puro (ADR-006: sem PyQt6).
"""
...
class TestBuildFeedbackBlock:
    def test_returns_none_below_four_negatives(self):
        rows = [_neg("resposta_errada") for _ in range(3)] + [_pos()]
    ...
    def test_generic_block_when_no_category_qualifies(self):
        # 4 negativos, mas nenhum motivo canônico com ≥3 ocorrências.
        rows = [_neg("resposta_errada"), _neg("incompleta"),
                _neg("fora_contexto"), _neg("")]
    ...
    def test_category_with_three_hits_qualifies(self):
        rows = [_neg("resposta_errada") for _ in range(3)] + [_neg("")]
        block = build_feedback_block(rows)
        assert block is not None
    ...
    def test_ceiling_of_two_categories_by_count(self):
        # Contagens distintas: as duas maiores entram; a terceira fica de fora.
        rows = ([_neg("generica") for _ in range(5)]
                + [_neg("fora_contexto") for _ in range(4)]
                + [_neg("resposta_errada") for _ in range(3)])
        block = build_feedback_block(rows)
        assert block is not None
    ...
    def test_tiebreak_follows_canonical_order(self):
        # Empate de contagem (3 cada): desempate pela ordem canônica
        # resposta_errada > incompleta > fora_contexto.
        rows = ([_neg("resposta_errada") for _ in range(3)]
                + [_neg("incompleta") for _ in range(3)]
    ...
    def test_free_text_does_not_become_category(self):
        # 4 negativos com texto livre "Outro…" — conta como negativo, mas
        # nenhuma categoria canônica qualifica → bloco genérico.
        rows = [_neg("Outro: a resposta inventou uma citação") for _ in range(4)]
        block = build_feedback_block(rows)
        assert block is not None
        assert _FRAG_GENERIC_BULLET in block
        assert _FRAG_ERRADA not in block
    ...
    def test_free_text_counts_toward_negatives_total(self):
        # 3 resposta_errada + 1 texto livre = 4 negativos → categoria qualifica.
        rows = ([_neg("resposta_errada") for _ in range(3)]
    ...
    def test_malformed_rows_are_ignored(self):
        ...
        # Apenas 3 negativos válidos → sem sinal suficiente, sem exceção.
        assert build_feedback_block(rows) is None
