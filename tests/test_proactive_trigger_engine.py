...
def test_trigger_engine_desligado():
    ...
    assert engine.should_trigger("Texto longo " * 50, 1, "Desligado") == False

def test_trigger_engine_curto_demais():
    engine = ProactiveTriggerEngine()
    # Não deve disparar se texto < 200 chars, mesmo no modo Estudo
    assert engine.should_trigger("Curto", 1, "Estudo") == False
...
def test_trigger_engine_leve():
    ...
    assert engine.should_trigger(long_text, 10, "Leve") == True
    
    # Se testar novamente logo em seguida (distância 1), deve ser False
    assert engine.should_trigger(long_text, 11, "Leve") == False
...
def test_trigger_engine_estudo():
    ...
    assert engine.should_trigger(long_text, 10, "Estudo") == True
    assert engine.should_trigger(long_text, 11, "Estudo") == True
    # Mesma página (dist 0) não dispara
    assert engine.should_trigger(long_text, 11, "Estudo") == False
